...
dia30_model <- coxme(Surv(DIA30, D30) ~ SD*DB  + transitT + BRTHW + diaARR + (1|LOAD/CalfID), 
                   data = FINAL_Analysis_filt)
summary(dia30_model)
...
nav30_model <- coxme(Surv(Nav30, N30) ~ SD*DB  + transitT + diaARR + BRTHW + (1|LOAD/CalfID), 
                   data = FINAL_Analysis_filt_n)
summary(nav30_model)
...
resp60_model <- coxme(Surv(PNEU60, P60) ~ SD*DB + transitT + diaARR + BRTHW + navelex + diaex + (1|LOAD/CalfID), data = FINAL_Analysis)
...
dead30_model <- coxme(Surv(DED30, D30yn) ~ SD + DB + transitT + diaARR + BRTHW +diaex + navelex + (1|LOAD/CalfID), 
                   data = FINAL_Analysis)
...
dead60_model <- coxme(Surv(DED60, D60yn) ~ SD + DB + diaARR + transitT + diaex + navelex + BRTHW + (1|LOAD/CalfID), 
                   data = FINAL_Analysis)
