...
FINAL7_new <- FINAL7_new %>% 
  mutate(
    Nav10 = if_else(navelex == 0,
                    coalesce(dfaN1, 10),
                    NA_real_))

FINAL7_new <- FINAL7_new %>% 
  mutate(
    N10 = if_else(Nav10 < 10, 1L, 0L))
...
FINAL_Analysis <- FINAL7_new %>% 
  dplyr::select(ID, sex, Breed2, stp, BRTHW, SORCE, SD, DB, diaload, diaARR, LOAD, week, transitT, PN1a, PN2a, DIA1, D30, P30, P60, D30yn, D60yn, diaex, navelex, DIA30, PNEU30, PNEU60, Nav10, N10, DED30, DED60, Remark_PNEU1, Remark_SCOURS1, Remark_DIED1)
...
FINAL_Analysis %>% 
  ...
  summarize(
    count = n(),
    prop_Nav = mean(N10, na.rm=TRUE), 
    sum_Nav  = sum(N10, na.rm=TRUE))
...
CrossTable (FINAL_Analysis$DIA1, FINAL_Analysis$N10)
...
#Navel Infection to 10d. 
FINAL_Analysis_filt_n <- FINAL_Analysis %>% 
  filter(navelex != 1)

surv_obj <- Surv(
  time  = FINAL_Analysis_filt_n$Nav10,
  event = FINAL_Analysis_filt_n$N10)
...
surv_obj <- Surv(
  time  = FINAL_Analysis_filt_nd$Nav10,
  event = FINAL_Analysis_filt_nd$N10)
...
surv_obj <- Surv(
  time  = FINAL_Analysis_filt_nb$Nav10,
  event = FINAL_Analysis_filt_nb$N10)
...
#removed diaARR as was not sig in model and did not change the estimate by more than 10%
...
dia30_model <- coxme(Surv(DIA30, D30) ~ SD*DB  + transitT + BRTHW + (1|LOAD/CalfID), 
                   data = FINAL_Analysis_filt)
summary(dia30_model)
...
overall <- emmeans(dia30_model, ~SD)
sd_contrasts <- pairs(overall, reverse = FALSE) 
summary(sd_contrasts, type = "response", infer = c(TRUE, TRUE))
...
#removed TransitT - changed estimate by 9%
#removed DiaARR - cahanged by 4%
...
nav10_model <- coxme(Surv(Nav10, N10) ~ SD*DB + BRTHW + (1|LOAD/CalfID), 
                   data = FINAL_Analysis_filt_n)
summary(nav10_model)
Anova(nav10_model, type = "III")

test_ph <- cox.zph(nav10_model)
print(test_ph)
...
interaction_means <- emmeans(nav10_model, ~SD|DB)
sd_contrasts <- pairs(interaction_means, reverse = FALSE) 
summary(sd_contrasts, type = "response", infer = c(TRUE, TRUE))
...
navel_30d <- coxme(Surv(Nav10, N10) ~ as.factor(SD) +  diaARR + transitT + BRTHW + diaex + (1|LOAD), FINAL_Analysis_filt_nd)
...
navel_30b <- coxme(Surv(Nav10, N10) ~ as.factor(SD) +  diaARR + transitT + BRTHW + diaex + (1|LOAD), FINAL_Analysis_filt_nb)
...
```{r}
#remove BW, no change in 3.5 estimate
#tranitT resulted in like 50% change so stays in. 
#remove diaEX, nothing happened which is weird bc we know that calves treated for scours are more likely to be treated for resp? 
#removed diaARR change in estimate by 2%


FINAL_Analysis$SD <- relevel(as.factor(FINAL_Analysis$SD), ref = "3")

resp60_model <- coxme(Surv(PNEU60, P60) ~ SD*DB + transitT  + navelex + (1|LOAD/CalfID), data = FINAL_Analysis)
...
dead30_model <- coxme(Surv(DED30, D30yn) ~ SD + DB + BRTHW + diaARR + diaex + (1|LOAD/CalfID), 
                   data = FINAL_Analysis)
...
#navelex = 3%
#diaex = 1%
#diarr was huge like 60%



FINAL_Analysis$SD <- relevel(as.factor(FINAL_Analysis$SD), ref = "3")

dead60_model <- coxme(Surv(DED60, D60yn) ~ SD + DB + diaARR + (1|LOAD/CalfID), 
                   data = FINAL_Analysis)
...
# 2. Create the dataset from the updated table
data <- data.frame(
  Outcome = c(
    rep("30d Scours Treatment", 6),
    rep("10d Navel Treatment", 6),
    rep("60d Respiratory Treatment", 6)
  ),
  Breed = rep(c(rep("Beef", 3), rep("Dairy", 3)), 3),
  Comparison = rep(c("3 vs. 3.5", "3 vs. 4.5", "3.5 vs. 4.5"), 6),
  HR = c(
    # Scours
    1.63, 1.24, 0.76,  0.86, 1.03, 1.19, 
    # Navel
    1.57, 2.35, 1.49,  0.58, 1.49, 2.56, 
    # Respiratory
    1.19, 1.14, 0.96,  0.79, 0.74, 0.93  
  ),
  Lower_CI = c(
    # Scours
    1.02, 0.78, 0.49,  0.51, 0.60, 0.71,
    # Navel
    0.58, 0.83, 0.55,  0.12, 0.21, 0.41,
    # Respiratory
    0.89, 0.86, 0.74,  0.51, 0.47, 0.59
  ),
  Upper_CI = c(
    # Scours
    2.61, 1.97, 1.16,  1.48, 1.75, 2.00,
    # Navel
    4.23, 6.67, 4.08,  2.89, 10.65, 15.91,
    # Respiratory
    1.59, 1.53, 1.24,  1.25, 1.17, 1.44
  ),
  p_value = c(
    # Scours
    0.039, 0.531, 0.279,  0.796, 0.992, 0.712,
    # Navel
    0.531, 0.126, 0.611,  0.710, 0.882, 0.452,
    # Respiratory
    0.322, 0.527, 0.921,  0.462, 0.270, 0.919
  )
)

# Define the exact visual sequence from TOP to BOTTOM
desired_order <- c(
  "Beef - 3 vs. 3.5", "Beef - 3 vs. 4.5", "Beef - 3.5 vs. 4.5",
  "Dairy - 3 vs. 3.5", "Dairy - 3 vs. 4.5", "Dairy - 3.5 vs. 4.5"
)

# Mutate Plot_Label into a factor with the sequence reversed for vertical plotting
data <- data %>%
  mutate(
    Plot_Label = paste(Breed, "-", Comparison),
    Plot_Label = factor(Plot_Label, levels = rev(desired_order))
  )

# 3. Generate the plot
ggplot(data, aes(x = HR, y = Plot_Label, color = Breed)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "grey50", linewidth = 0.8) +
  geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), height = 0.3, linewidth = 0.8) +
  geom_point(size = 2.5) +
  facet_grid(Outcome ~ ., scales = "free_y", space = "free_y") +
  scale_x_log10(breaks = c(0.1, 0.2, 0.5, 1, 2, 5, 10, 20)) +
  # Fixed the color mapping syntax here:
  scale_color_manual(values = c("Beef" = "#006400", "Dairy" = "black")) + 
  labs(
    x = "Hazard Ratio (Log Scale with 95% CI)",
    y = "",
    color = NULL # Removes the legend title entirely
  ) +
...
# 2. Create the dataset from the provided mortality table
mortality_data <- data.frame(
  Outcome = c(
    rep("Mortality to 30d", 3),
    rep("Mortality to 60d", 3)
  ),
  Comparison = rep(c("3 vs. 3.5", "3 vs. 4.5", "3.5 vs. 4.5"), 2),
  HR = c(
    0.86, 1.33, 1.55,  # 30d
    0.89, 1.23, 1.38   # 60d
  ),
  ...
  p_value = c(
    0.847, 0.616, 0.286, # 30d
    0.891, 0.699, 0.404  # 60d
  )
)

# Ensure the comparisons stay in a logical order on the Y-axis
mortality_data$Comparison <- factor(mortality_data$Comparison, 
                                    levels = c("3.5 vs. 4.5", "3 vs. 4.5", "3 vs. 3.5"))

# 3. Generate the forest plot
ggplot(mortality_data, aes(x = HR, y = Comparison)) +
  # Add reference line at HR = 1 (no difference in mortality risk)
  geom_vline(xintercept = 1, linetype = "dashed", color = "grey50", size = 0.8) +
  # Add confidence intervals
  geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), height = 0.2, size = 0.8, color = "#8db600") +
  # Add the Hazard Ratio points
  geom_point(size = 2.5, color = "#8db600") +
  # Facet by Outcome (30d vs 60d) stacked vertically
  facet_grid(Outcome ~ ., scales = "free_y", space = "free_y") +
  # Log scale for accurate geometric visualization of ratios
  scale_x_log10(breaks = c(0.4, 0.6, 0.8, 1, 1.5, 2, 2.5, 3, 3.5)) +
  # Customizing labels and clean styling
  labs(
    x = "Hazard Ratio (Log Scale with 95% CI)",
