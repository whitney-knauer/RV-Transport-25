...
# 2. Create the dataset from the provided table
data <- data.frame(
  Outcome = c(
  ...
  HR = c(
    1.65, 1.19, 0.72,  0.86, 1.04, 1.21, 
    #1.48, 2.39, 1.62,  0.51, 1.47, 2.88, 
    1.10, 1.13, 1.03,  0.84, 0.78, 0.93  
  ),
  Lower_CI = c(
    1.09, 0.79, 0.49,  0.55, 0.66, 0.77,
    #0.55, 0.83, 0.61,  0.10, 0.21, 0.46,
    0.87, 0.90, 0.83,  0.56, 0.52, 0.63
  ),
  Upper_CI = c(
    2.51, 1.78, 1.05,  1.36, 1.65, 1.89,
    #3.98, 6.95, 4.33,  2.55, 10.32, 18.28,
    1.40, 1.43, 1.27,  1.25, 1.17, 1.37
  ),
  p_value = c(
    0.014, 0.592, 0.097,  0.731, 0.977, 0.590,
    #0.625, 0.130, 0.479,  0.588, 0.890, 0.370,
    0.595, 0.420, 0.955,  0.553, 0.323, 0.896
  )
)

# Create a combined label for the Y-axis grouping
data <- data %>%
  mutate(Plot_Label = paste(Breed, "-", Comparison))

# 3. Generate the plot
ggplot(data, aes(x = HR, y = Plot_Label, color = Breed)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "grey50", size = 0.8) +
  geom_errorbarh(aes(xmin = Lower_CI, xmax = Upper_CI), height = 0.3, size = 0.8) +
  geom_point(size = 3.5) +
  facet_grid(Outcome ~ ., scales = "free_y", space = "free_y") +
  scale_x_log10(breaks = c(0.1, 0.5, 1, 2, 5, 10, 20)) +
  scale_color_manual(values = c("Beef" = "grey33", "Dairy" = "wheat")) +
  labs(
    title = "Hazard Ratios by Treatment Comparison",
    subtitle = "Comparing Beef vs. Dairy Across Three Health Outcomes",
...
ggplot(mortality_data, aes(x = HR, y = Comparison)) +
  ...
  labs(
    title = "Mortality Hazard Ratios by Treatment Comparison",
    subtitle = "Evaluating Risk Differences at 30 Days and 60 Days",
    x = "Hazard Ratio (Log Scale with 95% CI)",
