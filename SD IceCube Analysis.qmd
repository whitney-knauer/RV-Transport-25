...
custom_colors <- c("3.0" = "red4", "3.5" = "goldenrod2", "4.5" = "wheat4")
ggplot(data = subset (lying_data, !(Period =="Post" & StockingDensity == "3.5")), aes(x = Period, y = MeanTime, group = StockingDensity, color = StockingDensity)) +
  theme_classic () +
  geom_line() +   
  scale_color_manual(values=custom_colors)+
  # Draw lines
  geom_point() +                         # Add points at each data node
  geom_errorbar(aes(ymin = LowerCI, ymax = UpperCI), 
                width = 0.1, alpha = 0.7) +      # Add 95% CI error bars
  labs(
    y = "Lying Time (min/hr)",
  ...
  theme(text=element_text(size=12, family = "Times New Roman"))# Adjust Y-axis for better visibility
```
