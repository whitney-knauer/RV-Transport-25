...
mod_ar1 <- lme(
  fixed = lyingm ~ hoursfrom_fact * TRT + diaload  + BRTHW + TimeTrip ,
  random = ~ 1 | Cow,
  correlation = corAR1(form = ~ hours_num | Cow),
  data = analysis_data_full_f)
...
custom_colors <- c("DF3.0" = "#8db600", "DF3.5" = "#006400", "DF4.5" = "black")
intercept_0 <- which(levels(emm_df$hoursfrom_fact) == "0")
intercept_26<- which(levels(emm_df$hoursfrom_fact) == "26")

ggplot(emm_df, aes(x = hoursfrom_fact, y = emmean, color = TRT, linetype = TRT, group = TRT)) +
  geom_vline(xintercept = c(intercept_0, intercept_26), linetype = "dashed", color = "gray11") +
  geom_line() +
  geom_point() +
  scale_color_manual(values = custom_colors) +
  # Custom line types matching the sequence of your treatment levels:
  scale_linetype_manual(values = c("DF3.0" = "solid", "DF3.5" = "dashed", "DF4.5" = "dotdash")) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.3, linetype = "solid") +
  labs(
    x = "Hour from Transport (0)",
    y = "Lying Time (min/hr)",
    color = NULL,    # Removes the legend title for colors
    linetype = NULL  # Removes the legend title for line types so they merge cleanly
  ) +
  theme_classic() +
  scale_y_continuous(limits = c(0, 61), breaks = seq(0, 61, by = 10)) +
  scale_x_discrete(breaks = seq(-16, 50, by = 4)) +
  theme(
    legend.position = "top", 
...
lm_lay <-lmer(lyingm ~TRT*timecat + diaload + TimeTrip +  BRTHW + (1|Cow), data=analysis_data_full_f)
summary(lm_lay)
emmeans(lm_lay, ~TRT|timecat)
anova(lm_lay, type = 3)
plot(lm_lay)
...
mod_ar1 <- lme(
  fixed = standingm ~ hoursfrom_fact * TRT + diaload + BRTHW + TimeTrip,
  random = ~ 1 | Cow,
  correlation = corAR1(form = ~ hours_num | Cow),
  data = analysis_data_full_f)
...
lm_stand <-lmer(standingm ~TRT*timecat + diaload + BRTHW + TimeTrip + (1|Cow), data=analysis_data_full_f)
summary(lm_stand)
...
mod_ar1 <- lme(
  fixed = mi ~ hoursfrom_fact * TRT + diaload + BRTHW + TimeTrip,
  random = ~ 1 | Cow,
  correlation = corAR1(form = ~ hours_num | Cow),
  data = analysis_data_full_f)
...
lm_lay <-lmer(mi ~TRT*timecat + diaload + BRTHW + TimeTrip + (1|Cow), data=analysis_data_full_f)
summary(lm_lay)
emmeans(lm_lay, ~TRT|timecat)
plot(lm_lay)
...
# 1. Create the dataset for Lying Time only
lying_data <- data.frame(
  Period = factor(rep(c("Pre", "During", "Post"), each = 3), 
                  levels = c("Pre", "During", "Post")),
  StockingDensity = rep(c("3.0", "3.5", "4.5"), times = 3),
  MeanTime = c(
    # Pre (-12 to 0h)
    51.9, 51.3, 49.8,
    # During (0 to 26h)
...
# 2. Generate the line graph
#the data subset step removes the point on the graph for the SD 3.5 after as that only represents like 3 calves. 
custom_colors <- c("3.0" = "#8db600", "3.5" = "#006400", "4.5" = "black")

ggplot(data = subset(lying_data, !(Period == "Post" & StockingDensity == "3.5")), 
       aes(x = Period, y = MeanTime, group = StockingDensity, color = StockingDensity, linetype = StockingDensity)) +
  theme_classic() +
  geom_line() +   
  scale_color_manual(values = custom_colors) +
  # Adds your custom line types:
  scale_linetype_manual(values = c("3.0" = "solid", "3.5" = "dashed", "4.5" = "dotdash")) +
  geom_point() +                          
  # Keeps the error bars solid:
  geom_errorbar(aes(ymin = LowerCI, ymax = UpperCI), 
                width = 0.1, alpha = 0.7, linetype = "solid") +     
  labs(
    y = "Lying Time (min/hr)",
    color = NULL,
    linetype = NULL) +
